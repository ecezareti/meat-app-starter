    ...
    ApplicationErrorHandler.prototype.handleError = function (errorResponse) {
        ...
        if (errorResponse instanceof HttpErrorResponse) {
            this.zone.run(function () {
                var httpError = errorResponse.status;
                switch (httpError) {
                    case 401:
                        _this.injector.get(LoginService).handleLogin();
                        break;
                    case 403:
                        _this.notificationService.notify(errorResponse.error.message || 'Não autorizado');
                        break;
                    case 404:
                        _this.notificationService.notify(errorResponse.error.message || 'Recurso não encontrado');
                        break;
                }
            });
        }
